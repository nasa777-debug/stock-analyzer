...
class Transaction(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'user_id': self.user_id, 'ticker': self.ticker, 'action': self.action, 'shares': float(self.shares), 'price': float(self.price), 'total_value': float(self.shares*self.price), 'timestamp': self.timestamp.isoformat()}
